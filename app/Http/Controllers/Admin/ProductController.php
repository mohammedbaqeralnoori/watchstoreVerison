...
class ProductController extends Controller
{
    ...
    {
        $image = Product::saveImage($request->file('file'));
        $product = Product::query()->create([
            'title' => $request->input('title'),
            'title_en' => $request->input('title_en'),
            'slug' => Helper::make_slug($request->input('title')),
            'price' => $request->input('price'),
            'prview' => $request->input('prview', 0),
        ...
        return redirect()->route('products.index')->with('message', 'محصول با موفقيت اضاقه شد');
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        return redirect()->route('products.index')->with('message', 'محصول با موفقيت ويرايش شد');
    }
    ...
    public function addPropertie($id)
    {
        $product = Product::query()->find($id);
        $property_groups = PropertyGroup::query()->get();
        return view('admin.product.create_property', compact('property_groups', 'product'));
    }
    public function storePropertie(Request $request, $id)
    {
                $product = Product::query()->find($id);
                $product->properties()->sync($request->properties);
                return redirect()->route('products.index')->with('message', 'ويزكي ها  با موفقيت اضافه شد');
